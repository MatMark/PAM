...
public class BluetoothConnectionService {
    private static final String TAG = "BlConnServ";
    private static final String APP_NAME = "RPiCtrl";
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                ...
                try {
                    if (!mmSocket.isConnected()) blReceiver("Can't connect");
                    bytes = mmInStream.read(buffer);
                    String incomingMessage = new String(buffer, 0, bytes);
                    Log.d(TAG, "InputStream: " + incomingMessage);
                    blReceiver(incomingMessage);

                } catch (IOException e) {
                    Log.e(TAG, "Error reading Input Stream. " + e.getMessage());
                    mmInStream = null;
                    mmOutStream = null;
                    blReceiver("Lost connection");
                    break;
                }
            }
        }

        private void blReceiver(String text) {
            // The string "my-message" will be used to filer the intent
            Intent intent = new Intent("my-message");
            // Adding some data
            intent.putExtra("my-text", text);
        ...
        public void write(byte[] bytes) {
            ...
            try {
                ...
                blReceiver("You are not connected");
            }
        }
    ...
    public boolean isConnected() {
        ...
        if (mConnectedThread.mmSocket == null) return false;
        if (mConnectedThread.mmInStream == null) return false;
